...
data class OverdraftFeeChargeCapInner(
  ...
  enum class FeeType(@get:JsonValue val value: kotlin.String) {
    ...
    companion object {
      ...
      fun forValue(value: kotlin.String): FeeType = values().firstOrNull { it -> it.value == value }
        ?: throw IllegalArgumentException("Unexpected value '$value' for enum 'OverdraftFeeChargeCapInner'")
    }
  }
  ...
  enum class MinMaxType(@get:JsonValue val value: kotlin.String) {
    ...
    companion object {
      @JvmStatic
      @JsonCreator
      fun forValue(value: kotlin.String): MinMaxType = values().firstOrNull { it -> it.value == value }
        ?: throw IllegalArgumentException("Unexpected value '$value' for enum 'OverdraftFeeChargeCapInner'")
    }
  }
  ...
  enum class CappingPeriod(@get:JsonValue val value: kotlin.String) {
    ...
    companion object {
      @JvmStatic
      @JsonCreator
      fun forValue(value: kotlin.String): CappingPeriod = values().firstOrNull { it -> it.value == value }
        ?: throw IllegalArgumentException("Unexpected value '$value' for enum 'OverdraftFeeChargeCapInner'")
    }
  }
}
