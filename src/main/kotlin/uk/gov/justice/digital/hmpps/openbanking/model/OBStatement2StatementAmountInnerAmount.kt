 ...
 * @param subType The amount in the domestic or base accounting currency. Default is Base Currency (BCUR) if not specified
 */
data class OBStatement2StatementAmountInnerAmount(
    ...
    @Schema(example = "null", description = "The amount in the domestic or base accounting currency. Default is Base Currency (BCUR) if not specified")
    @get:JsonProperty("SubType") val subType: OBStatement2StatementAmountInnerAmount.SubType? = SubType.BCUR
) {
        ...
        companion object {
            ...
            fun forValue(value: kotlin.String): SubType {
                return values().firstOrNull{it -> it.value == value}
                    ?: throw IllegalArgumentException("Unexpected value '$value' for enum 'OBStatement2StatementAmountInnerAmount'")
            }
        }
    }
