 ...
 * @param applicationFrequency How often is interest applied to the PCA for this tier/band i.e. how often the financial institution pays accumulated interest to the customer's PCA.
 ...
 * @param identification Unique and unambiguous identification of a  Tier Band for a PCA.
 ...
 * @param bankInterestRateType Interest rate types, other than AER, which financial institutions may use to describe the annual interest rate payable to the PCA.
 * @param bankInterestRate Bank Interest for the PCA product
...
data class TierBandInner1(
    ...
    @Schema(example = "null", required = true, description = "How often is interest applied to the PCA for this tier/band i.e. how often the financial institution pays accumulated interest to the customer's PCA.")
    @get:JsonProperty("ApplicationFrequency", required = true) val applicationFrequency: TierBandInner1.ApplicationFrequency,

    @Schema(example = "null", required = true, description = "Type of interest rate, Fixed or Variable")
    @get:JsonProperty("FixedVariableInterestRateType", required = true) val fixedVariableInterestRateType: TierBandInner1.FixedVariableInterestRateType,
    ...
    @Schema(example = "null", description = "Unique and unambiguous identification of a  Tier Band for a PCA.")
    ...
    @get:JsonProperty("CalculationFrequency") val calculationFrequency: TierBandInner1.CalculationFrequency? = null,

    @Schema(example = "null", description = "Amount on which Interest applied.")
    @get:JsonProperty("DepositInterestAppliedCoverage") val depositInterestAppliedCoverage: TierBandInner1.DepositInterestAppliedCoverage? = null,

    @Schema(example = "null", description = "Interest rate types, other than AER, which financial institutions may use to describe the annual interest rate payable to the PCA.")
    @get:JsonProperty("BankInterestRateType") val bankInterestRateType: TierBandInner1.BankInterestRateType? = null,

    @get:Pattern(regexp="^(-?\\d{1,3}){1}(\\.\\d{1,4}){0,1}$")
    @Schema(example = "null", description = "Bank Interest for the PCA product")
