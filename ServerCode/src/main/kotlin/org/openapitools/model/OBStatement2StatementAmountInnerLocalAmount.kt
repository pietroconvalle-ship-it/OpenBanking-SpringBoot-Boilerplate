 ...
 * @param subType The amount in the local market currency for which the asset is held. Default is Local Currency (LCUR) if not specified
 */
data class OBStatement2StatementAmountInnerLocalAmount(
    ...
    @Schema(example = "null", description = "The amount in the local market currency for which the asset is held. Default is Local Currency (LCUR) if not specified")
    @get:JsonProperty("SubType") val subType: OBStatement2StatementAmountInnerLocalAmount.SubType? = SubType.LCUR
) {
